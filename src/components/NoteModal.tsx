...
export function NoteModal({ note, isOpen, onClose, onSave, onDelete }: NoteModalProps) {
  ...
  const performSave = (isClosing: boolean = false) => {
    ...
    const currentStr = JSON.stringify({ t: currentData.title, c: currentData.content, m: currentData.metadata, p: currentData.projectId, l: currentData.labelId });
  ...
  const createNewLabel = async () => {
    if (projectId === 'none' || !db || !user) return;
