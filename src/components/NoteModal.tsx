...
export function NoteModal({ note, isOpen, onClose, onSave }: NoteModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleSave()}>
      <DialogContent className="sm:max-w-[900px] w-[95vw] max-h-[95vh] flex flex-col p-0 border-none rounded-2xl overflow-hidden z-[100] bg-background shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto pt-6 pb-20 space-y-6">
          <div className="px-10">
            <Input
              ...
              className="border-none shadow-none focus-visible:ring-0 text-3xl font-bold px-0 bg-transparent h-auto"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 px-10">
            ...
            {labels.map(label => (
              <Badge key={label} variant="secondary" className="flex items-center gap-1.5 rounded-md px-2 py-0.5">
                {label}
        ...
        <div className="p-4 bg-card border-t flex justify-end items-center px-10">
          <Button onClick={handleSave} className="rounded-lg px-8 font-bold text-sm">Done</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
